-- =====================================================================
-- SPRINT 17 — LESSON PROGRESS ENUM CAST FIX (42804)
-- Root cause: CASE pada UPDATE ... SET status = CASE ... 'completed' ...
-- menghasilkan text, sedangkan kolom bertipe public.lesson_progress_status.
-- Fix: cast eksplisit setiap branch CASE ke public.lesson_progress_status.
-- Tidak mengubah nama function, signature, tabel, enum, RLS, trigger.
...
begin
  ...
  insert into public.lesson_progress (
    ...
    v_user, v_tenant, p_lesson_id,
    'in_progress'::public.lesson_progress_status, now(), now()
  )
  ...
  v_was_completed := v_row.status = 'completed'::public.lesson_progress_status;
  ...
  update public.lesson_progress set
    ...
    status = case
      when v_was_completed or p_complete or (v_total > 0 and v_done >= v_total)
...
-- Grants tetap sama (SECURITY DEFINER tidak boleh anon/PUBLIC).
